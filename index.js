...
function drawParticles() {
  ...
  particles.forEach((p, i) => {
    ...
    p.y   += p.vy;
    p.life++;
    if (p.life > p.max) particles[i] = spawnParticle();
...
function runIntro() {
  ...
  setTimeout(() => introLogo.classList.add('animate'),    300);
  setTimeout(() => introTagline.classList.add('animate'), 800);
...
function buildGrid() {
  ...
  songs.forEach((song, idx) => {
    ...
    card.addEventListener('click', () => openVideo(song));
    card.addEventListener('keydown', e => { if (e.key === 'Enter' || e.key === ' ') openVideo(song); });
...
function openVideo(song) {
  ...
  mainVideo.src = song.file + '.mp4';
  mainVideo.load();

  videoOverlay.classList.remove('hidden', 'closing');
  videoOverlay.classList.add('visible');
  isVideoOpen = true;

  // Show title after a beat
  setTimeout(() => videoTitleWrap.classList.add('visible'), 400);
...
mainVideo.addEventListener('loadedmetadata', () => {
  totalTimeEl.textContent = formatTime(mainVideo.duration);
});
...
videoOverlay.addEventListener('click', e => {
  ...
  if (e.target.closest('.video-controls') || e.target.closest('.close-btn')) return;
  if (videoUi.classList.contains('hide-controls')) {
    videoUi.classList.remove('hide-controls');
    scheduleHideControls();
  } else {
    clearTimeout(hideControlsTimer);
...
document.addEventListener('keydown', e => {
  ...
  if (e.key === 'ArrowRight')  { mainVideo.currentTime += 10; resetControls(); scheduleHideControls(); }
  if (e.key === 'ArrowLeft')   { mainVideo.currentTime -= 10; resetControls(); scheduleHideControls(); }
